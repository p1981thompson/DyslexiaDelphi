...
qualtrics.Terminology.data<-read_excel("H:/DVMB/CREWS/test data/CATALISE_terminology.xlsx",sheet="CATALISE_T")
...
processInput_T_Anon <- function(panel.member) { 
  temp <<- environment()
  Sweave(paste(getwd(),'/reports/delphi_CREWS_template_anonymous.Rnw',sep=""), output = paste(getwd(),'/reports/Delphi_CREWS_report_anonymous.tex', sep = ''))
