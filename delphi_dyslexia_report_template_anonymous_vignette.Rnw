...
<<results=tex,echo=FALSE>>=
  require(xtable)
  require(readxl)
delphi.T.comments<-read_excel("/Volumes/PSYHOME/PSYRES/pthompson/DVMB/CREWS_delphi/test data/vignette data/vignette_data_text.xlsx",sheet="Comments")
...
<<results=hide,echo=FALSE>>=
require(readxl)
vignette_scores<-read_excel("/Volumes/PSYHOME/PSYRES/pthompson/DVMB/CREWS_delphi/test data/vignette data/QuizResponses.xlsx", sheet="Sheet")

##############################

ggplot(data.frame(RECAP_score=vignette_scores$Points_P1),aes(x=factor(RECAP_score,levels=c(0:16))))+
...
ggplot(data.frame(RECAP_score=vignette_scores$Points_P2),aes(x=factor(RECAP_score,levels=c(0:16))))+
  geom_bar(aes(fill=RECAP_score,y = (..count..)/sum(..count..))) +
  geom_text(aes(y = ((..count..)/sum(..count..)), label = scales::percent((..count..)/sum(..count..))), stat = "count", vjust = -0.25) +
  labs(y = "Percent", x = "RECAP Score")+theme_bw()+theme(legend.position="none")+ scale_x_discrete(drop=FALSE)
...
ggplot(data.frame(RECAP_score=vignette_scores$Points_P3),aes(x=factor(RECAP_score,levels=c(0:16))))+
  geom_bar(aes(fill=RECAP_score,y = (..count..)/sum(..count..))) +
  geom_text(aes(y = ((..count..)/sum(..count..)), label = scales::percent((..count..)/sum(..count..))), stat = "count", vjust = -0.25) +
  labs(y = "Percent", x = "RECAP Score")+theme_bw()+theme(legend.position="none")+ scale_x_discrete(drop=FALSE)
