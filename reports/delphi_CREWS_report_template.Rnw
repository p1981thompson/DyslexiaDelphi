...
##############################
  ...
  statements_T1_ind<-c(seq(8,59,2))
  comments_ind_T<-c(seq(9,59,2),60)
  
  contact.data.T<-qualtrics.terminology.Test[,1:6]
  #add.info2<-qualtrics.data.Test2[,7:19]
  #r.data<-
  v.data.T<-data.frame(matrix(0, nrow = dim(qualtrics.terminology.Test)[1], ncol = 26))
  comments.T<-data.frame(matrix(0, nrow = dim(qualtrics.terminology.Test)[1], ncol = 27))
...
names(delphi.Term.combine)<-delphi.Terminology.statements[1:26,2]
