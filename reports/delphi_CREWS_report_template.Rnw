...
likert.density.plot3(Q29A, type="density",legend.position="bottom",wrap=110)+ geom_vline(xintercept = as.numeric(delphi.Term.combine.N[delphi.Term.combine2$username==panel.member,29]), colour = "red", size=1.5) +theme_bw()+ theme(legend.position="none", axis.text.x = element_text(angle = 30, hjust = 1))}
@
